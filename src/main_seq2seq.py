"""
Train Seq2Seq LSTM (one model, predicts all horizons at once) on the SOFC
voltage dataset. Meant for Colab (GPU) -- see
notebooks/SOFC_Colab_Forecasting.ipynb, which just cd's into src/ and runs
...
def main():
    ...
    os.makedirs(os.path.join(OUTPUT_DIR, "models_saved", "seq2seq"), exist_ok=True)
    os.makedirs(os.path.join(OUTPUT_DIR, "predictions_cache", "seq2seq"), exist_ok=True)
    os.makedirs(os.path.join(OUTPUT_DIR, "reports"), exist_ok=True)

    model.save(os.path.join(OUTPUT_DIR, "models_saved", "seq2seq", "model.pt"))
    np.savez(os.path.join(OUTPUT_DIR, "predictions_cache", "seq2seq", "predictions.npz"), y_true=yw_te, y_pred=y_pred)

    results_df = pd.DataFrame(results)[["horizon", "MAE", "RMSE", "R2", "DTW"]]
    results_df.to_csv(os.path.join(OUTPUT_DIR, "reports", "seq2seq_results.csv"), index=False)
    print(results_df)
