"""
Experiment 3, LSTM (notes/SOFC_data_notes.md section 28-29): does forecasting
V(t+h) improve if I(t+h) is given as a KNOWN exogenous input, instead of
forecast blind? Mirrors main_lstm_delta.py exactly, plus one extra feature
channel (I(t+h), broadcast across the window -- see
windowing.augment_with_future_exogenous()) that a real MPC controller would
already know (it's the controller's own planned current setpoint).

The RF/XGBoost version of this experiment (main_v_given_i.py, local) found
a real but horizon-limited effect: helps clearly at h=1, mixed/negative from
h>=10, likely because flat-vector concatenation is too crude to exploit a
single distant exogenous value. This version keeps the temporal window
intact (broadcast, not flattened) to see if that changes the picture. Meant
for Colab (GPU).
"""
import os
import sys
...
def main():
    ...
    for h in FORECAST_HORIZONS:
        ...
        os.makedirs(os.path.join(OUTPUT_DIR, "models_saved", "lstm_delta_given_i"), exist_ok=True)
        os.makedirs(os.path.join(OUTPUT_DIR, "predictions_cache", "lstm_delta_given_i"), exist_ok=True)
        model.save(os.path.join(OUTPUT_DIR, "models_saved", "lstm_delta_given_i", f"h{h}.pt"))
        np.savez(os.path.join(OUTPUT_DIR, "predictions_cache", "lstm_delta_given_i", f"h{h}.npz"), y_true=y_true, y_pred=y_pred)

    os.makedirs(os.path.join(OUTPUT_DIR, "reports"), exist_ok=True)
    results_df = pd.DataFrame(results)[["horizon", "MAE", "RMSE", "R2", "DTW"]]
    results_df.to_csv(os.path.join(OUTPUT_DIR, "reports", "lstm_delta_given_i_results.csv"), index=False)
    print(results_df)
