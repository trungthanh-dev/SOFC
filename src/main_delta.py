"""
Delta-Target Reformulation for Random Forest + XGBoost: train on
y(t+h) - y(t) instead of the raw future value, reconstruct
y_hat(t+h) = anchor + delta_hat before evaluating. Raw-target baseline
results (main.py) are left untouched -- this writes to separate
*_delta_results.csv / *_delta/ paths, same convention as FCF's
main_rf_poseidon_delta.py / main_xgb_poseidon_delta.py.

Runs locally, no GPU needed. See main_lstm_delta.py / main_tcn_delta.py /
main_seq2seq_delta.py for the neural-net delta variants (Colab).
"""
import os
import sys
import time
...
def main():
    ...
    model_families = [
        ("random_forest_delta", "rf_delta", RandomForestModel, dict(n_estimators=300)),
        ("xgboost_delta", "xgb_delta", XGBoostModel, dict(n_estimators=300)),
    ]
