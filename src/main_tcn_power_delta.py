"""
Delta-Target Reformulation for TCN, Power (W) target: train on
W(t+h) - W(t) instead of the raw future value, reconstruct
y_hat(t+h) = anchor + delta_hat before evaluating. Raw-target baseline
(main_tcn_power.py) is left untouched. Mirrors main_tcn_delta.py (voltage
...
def main():
    ...
    for h in FORECAST_HORIZONS:
        ...
        os.makedirs(os.path.join(OUTPUT_DIR, "models_saved", "tcn_power_delta"), exist_ok=True)
        os.makedirs(os.path.join(OUTPUT_DIR, "predictions_cache", "tcn_power_delta"), exist_ok=True)
        model.save(os.path.join(OUTPUT_DIR, "models_saved", "tcn_power_delta", f"h{h}.pt"))
        np.savez(os.path.join(OUTPUT_DIR, "predictions_cache", "tcn_power_delta", f"h{h}.npz"), y_true=y_true, y_pred=y_pred)

    os.makedirs(os.path.join(OUTPUT_DIR, "reports"), exist_ok=True)
    results_df = pd.DataFrame(results)[["horizon", "MAE", "RMSE", "R2", "DTW"]]
    results_df.to_csv(os.path.join(OUTPUT_DIR, "reports", "tcn_power_delta_results.csv"), index=False)
    print(results_df)
