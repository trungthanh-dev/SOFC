"""
Train LSTM (direct forecast, one model per horizon) to forecast Power (W)
instead of Voltage (V) -- same pipeline as main_lstm.py, just a different
...
def main():
    ...
    for h in FORECAST_HORIZONS:
        ...
        os.makedirs(os.path.join(OUTPUT_DIR, "models_saved", "lstm_power"), exist_ok=True)
        os.makedirs(os.path.join(OUTPUT_DIR, "predictions_cache", "lstm_power"), exist_ok=True)
        model.save(os.path.join(OUTPUT_DIR, "models_saved", "lstm_power", f"h{h}.pt"))
        np.savez(os.path.join(OUTPUT_DIR, "predictions_cache", "lstm_power", f"h{h}.npz"), y_true=yw_te, y_pred=y_pred)

    os.makedirs(os.path.join(OUTPUT_DIR, "reports"), exist_ok=True)
    results_df = pd.DataFrame(results)[["horizon", "MAE", "RMSE", "R2", "DTW"]]
    results_df.to_csv(os.path.join(OUTPUT_DIR, "reports", "lstm_power_results.csv"), index=False)
    print(results_df)
