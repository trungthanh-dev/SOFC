"""
Train TCN (direct forecast, one model per horizon) on the SOFC voltage
dataset. Meant for Colab (GPU) -- see notebooks/SOFC_Colab_Forecasting.ipynb,
which just cd's into src/ and runs `python main_tcn.py`.
"""
import os
import sys
...
def main():
    ...
    for h in FORECAST_HORIZONS:
        ...
        os.makedirs(os.path.join(OUTPUT_DIR, "models_saved", "tcn"), exist_ok=True)
        os.makedirs(os.path.join(OUTPUT_DIR, "predictions_cache", "tcn"), exist_ok=True)
        model.save(os.path.join(OUTPUT_DIR, "models_saved", "tcn", f"h{h}.pt"))
        np.savez(os.path.join(OUTPUT_DIR, "predictions_cache", "tcn", f"h{h}.npz"), y_true=yw_te, y_pred=y_pred)

    os.makedirs(os.path.join(OUTPUT_DIR, "reports"), exist_ok=True)
    results_df = pd.DataFrame(results)[["horizon", "MAE", "RMSE", "R2", "DTW"]]
    results_df.to_csv(os.path.join(OUTPUT_DIR, "reports", "tcn_results.csv"), index=False)
    print(results_df)
