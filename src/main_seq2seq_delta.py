"""
Delta-Target Reformulation for Seq2Seq LSTM: train on
y(t+h) - y(t) (per horizon) instead of raw future values, reconstruct
...
baseline (main_seq2seq.py) is left untouched. Meant for Colab (GPU).
"""
import os
import sys
...
def main():
    ...
    os.makedirs(os.path.join(OUTPUT_DIR, "models_saved", "seq2seq_delta"), exist_ok=True)
    os.makedirs(os.path.join(OUTPUT_DIR, "predictions_cache", "seq2seq_delta"), exist_ok=True)
    os.makedirs(os.path.join(OUTPUT_DIR, "reports"), exist_ok=True)

    model.save(os.path.join(OUTPUT_DIR, "models_saved", "seq2seq_delta", "model.pt"))
    np.savez(os.path.join(OUTPUT_DIR, "predictions_cache", "seq2seq_delta", "predictions.npz"), y_true=y_true, y_pred=y_pred)

    results_df = pd.DataFrame(results)[["horizon", "MAE", "RMSE", "R2", "DTW"]]
    results_df.to_csv(os.path.join(OUTPUT_DIR, "reports", "seq2seq_delta_results.csv"), index=False)
    print(results_df)
